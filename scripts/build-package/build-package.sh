...
mkdir -p "$PACKAGE_DIR/dist/shared"

cp -r "$REPO_DIR/packages/shared/lib/." "$PACKAGE_DIR/dist/shared/"
cp -r "$REPO_DIR/apps/api/dist/." "$PACKAGE_DIR/dist/api/"
cp -r "$REPO_DIR/apps/web/dist/." "$PACKAGE_DIR/dist/web/"
cp -r "$REPO_DIR/apps/api/drizzle" "$PACKAGE_DIR/dist/drizzle"

node "$REPO_DIR/scripts/build-package/rewrite-shared-imports.mjs"
